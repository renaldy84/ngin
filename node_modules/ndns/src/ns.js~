...
Client.prototype.add = function (zone, name, type) {
	...
	this.query("lpush", ["ndns", "question", name], arguments[i]);
	this._emit.push("add");
    }
};
...
function Server () {
    ...
    self.db.onRR = function (rr) {
	self._res[0].addRR(rr);
	self.emit("rr", rr);
    };

    self.db.onRRend = function () {
	self.res.shift.send();
	self.emit("rrEnd");
    };

    self.on("request", respond);
};
...
Server.prototype.add = function () {
    this.db.add.apply(this.db, arguments);
};
