...
Proxy.prototype.listen = function(port) {
  ...
  self.server = http.createServer(function(request, response) {
    ...
    if(!request.headers.host) {
      ...
      self.writeResponse(response, 'No location header', 500);
      return;
    }
    
    self.emit('request', request.method + ' ' + request.headers.host + request.url);

    var turl = url.parse('http://' + self.domains[request.headers.host.split(':').shift()] + request.url);
    ...
    if(!turl) {
      ...
      self.writeResponse(response, 'No route matches', 500);
      return;
    }
...
Proxy.prototype.writeResponse = function(response, jsonBody, statusCode) {
