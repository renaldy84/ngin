...
var Application = module.exports = function(directory, port) {
  ...
  if(!path.existsSync(path.join(directory, 'package.json'))) {
    console.log(directory + ' is not valid node.js application. Please create package.json using npm init.');
    continue;
  }
...
Application.prototype.prepare = function() {
  ...
  // Move to the application directory to install npm modules on the right path.
