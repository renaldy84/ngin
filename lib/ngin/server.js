...
var Server = module.exports = function(options) {
  ...
  process.on('SIGINT', function() { self.kill("SIGINT"); });
  process.on('SIGTERM', function() { self.kill("SIGTERM"); });
  process.on('SIGQUIT', function() { self.kill("SIGQUIT"); });    
  process.on('exit', function(code) { self.emit('close'); });
}
...
Server.prototype.startApps = function() {
  ...
  self.emit('log', 'info', 'Starting applications ...');
...
Server.prototype.startApp = function(name) {
  ...
  self.emit('log', 'info', '=> Starting app ' + name);
  app.prepare();
}
